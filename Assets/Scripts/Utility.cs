...
public static class Utility
{
    ...
    public static void DrawBezier(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, LineRenderer lineRenderer)
    {
        float t = 0.0f;
        for (int i = 0; i < lineRenderer.positionCount; i++)
        {
            Vector3 p = EvaluateBezier(p0, p1, p2, p3, t);
            lineRenderer.SetPosition(i, p);
            t += (1.0f / (float)lineRenderer.positionCount);
        }
    }

    public static void DrawCatmull(Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3, LineRenderer lineRenderer)
    {
        float t = 0.0f;
        for (int i = 0; i < lineRenderer.positionCount; i++)
        {
            Vector3 p = EvaluateCatmull(p0, p1, p2, p3, t);
            lineRenderer.SetPosition(i, p);
            t += (1.0f / (float)lineRenderer.positionCount);
        }
    }
